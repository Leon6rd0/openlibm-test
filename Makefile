...
# --- 核心改动 1: 定义默认目标只为 Build ---
.PHONY: all build run check clean cleanall clean-reports
...
# --- 核心改动 2: 明确定义 Build 目标 ---
# build 只依赖二进制文件，不依赖 .err 文件
build: $(BINS) $(LIBS)

# --- 核心改动 3: Check 目标 ---
# check 依赖 REPORT，如果 REPORT 缺失会自动触发运行
# 如果 REPORT 存在，直接 grep 结果
check: $(B)/REPORT
	@grep FAIL $< || echo PASS

# Error when libm not passed tests (保留旧习惯)
checklibm: check
